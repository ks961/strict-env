...
const { Validator } = require("@d3vtool/validator");
const { eValidator } = require("../evalidator.cjs");
const { acceptedEnvFiles } = require('../acceptedEnvFiles.cjs');
 ...
 * @param {Object | ObjectValidator } [options.validators] - A list of validator functions to apply to the environment variables once loaded. Optional.
 * 
...
function setup(options) {
    ...
    if(options && options?.file) {
        if (!fs.existsSync(options.file)) {
            throw new Error(`No Env file found name '${options.file}', please add one.`);
        }
